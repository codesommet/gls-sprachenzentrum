...
class QuizQuestionsSeeder extends Seeder
{
    public function run(): void
    {
        DB::transaction(function () {
                    ...
                    'questions' => [
                        [
                            ...
                            'options' => [
                                ...
                                ['option_text' => 'heißen', 'is_correct' => false,'sort_order' => 3],
                                ['option_text' => 'heißt du', 'is_correct' => false,'sort_order' => 4],
                            ],
                        ],
                        [
            ...
            // 1) Create/Update quizzes
            $quizIds = [];
            foreach ($quizzes as $level => $quiz) {
                // upsert quiz by unique level
                ...
                $quizIds[$level] = DB::table('quizzes')->where('level', $level)->value('id');
            }

            // 2) Re-seed questions/options per quiz (safe)
            //    We remove existing questions for these quizzes to avoid duplicates
            $ids = array_values($quizIds);

            // Delete options of these quizzes questions
            $questionIds = DB::table('quiz_questions')->whereIn('quiz_id', $ids)->pluck('id');
            if ($questionIds->count() > 0) {
                DB::table('quiz_options')->whereIn('question_id', $questionIds)->delete();
            }

            // Delete questions
            DB::table('quiz_questions')->whereIn('quiz_id', $ids)->delete();

            // Insert fresh questions/options
            foreach ($quizzes as $level => $quiz) {
